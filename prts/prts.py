...
# HEADERS = ['stage', 'enemy', 'image_urls', 'extra_image_urls', 'map_url']
HEADERS = ['enemy_img_url']
my_table = pd.DataFrame(columns=HEADERS)
BASE_URL = 'http://prts.wiki/w/'
IMG_BASE_URL = 'http://prts.wiki/images'
...
# for i, row in stage_table.iterrows():
	try:
		...
		# stage = row['stage']
		# print(stage)
		browser.get(BASE_URL + stage)
		sleep(3)
		parser = etree.HTML(browser.page_source)
	# if parser.xpath('//table[@class="wikitable"]'):
	# 	table_normal = parser.xpath('//table[@class="wikitable"]')[0]
	# else:
	# 	print(stage)
	# 	continue
	...
	# try:
	# 	# number of enemies
	# 	enemy_box = table_normal.xpath('.//td[@colspan="2"]')[6]
	# 	enemy = enemy_box.xpath('./text()')[0]
	# 	# for i, each in enumerate(enemy):
	# 	# 	print(each.xpath('./text()')[0])

	# 	# imgs
	# 	if table_normal.xpath('.//td[@colspan="3"]//div[@class="nomobile"]//img'):
	# 		imgs = table_normal.xpath('.//td[@colspan="3"]//div[@class="nomobile"]//img')
	# 	elif table_normal.xpath('.//td[@colspan="2"]//div[@class="nomobile"]//img'):
	...
	data = {
			# "stage": stage, "enemy": enemy, "image_urls": images, 
...
browser.close()
my_table.to_csv(r'./data/enemy_imgs_2.csv', header=HEADERS, index=None, sep=',', mode='a')
