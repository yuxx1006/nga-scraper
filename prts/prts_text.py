...
BASE_URL = 'http://prts.wiki/w/%E5%88%86%E7%B1%BB:%E5%AF%BB%E8%AE%BF%E6%A8%A1%E6%8B%9F'
WEB_BASE_URL = 'http://prts.wiki/w/'
IMG_BASE_URL = "http://prts.wiki"

DIS_BASE_URL = 'http://prts.wiki/index.php?title={}&action=edit'
...
for i, row in stage_table.iterrows():
	try:
		if i == 0: continue
		# stage_url = row['url']
		stage_url = DIS_BASE_URL.format(row['name'])
		browser.get(stage_url)
		sleep(5)
		parser = etree.HTML(browser.page_source)
		# imgs = []
		print(row['name'])
		if parser.xpath('//div[@class="mw-body-content"]//div[@id="mw-content-text"]'):
			text = parser.xpath('//textarea[@id="wpTextbox1"]/text()')[0]
			print(text)
...
browser.close()
my_table.to_csv(r'./data/prts_text.csv', header=HEADERS, index=None, sep=',', mode='a')
